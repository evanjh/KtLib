...
open class TdHandler : AbsEvents {
    ...
    open fun writePersist(userId: Int, persistId: Int, subId: Long, vararg data: Any?, allowFunction: Boolean = false, allowCancel: Boolean = true, dontSave: Boolean = false) {
        ...
        val persist = TdPersist(userId, persistId, subId, arrayOf(* data), allowFunction, allowCancel, dontSave = dontSave)

        val cache = sudo.persists.fetch(userId)

        cache.tdPersist = persist
        cache.changed = true
    ...
    suspend infix fun removePersist(userId: Int) {
        ...
        if (cache.tdPersist != null) {

            cache.tdPersist = null
            cache.changed = true
