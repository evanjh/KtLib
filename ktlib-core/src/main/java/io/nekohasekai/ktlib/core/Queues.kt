...
class FastQueue<T>(private val capacity: Int, private val onClear: (() -> Unit)?) :
    LinkedBlockingQueue<T>(Int.MAX_VALUE) {
...
fun mkFastPool(capacity: Int = 1, onClear: (() -> Unit)? = null) =
    ThreadPoolExecutor(1, 1, 0L, TimeUnit.MILLISECONDS, mkFastQueue(capacity, onClear))
